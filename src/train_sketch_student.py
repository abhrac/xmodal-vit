...
def main():
    ...
    device = p.sketch_student_device
    teacher_dir = os.path.join(p.teacher_tgt_dir, 'teacher_20000.pth')
    os.makedirs(p.sketch_student_tgt_dir, exist_ok=True)
    ...
    sketch2xat_student.encoder = timm.create_model(p.student_encoder, pretrained=True).to(device)
    ...
    weight_decay = 1e-5
    fp16_precision = True
    optimizer = torch.optim.Adam(sketch2xat_student.parameters(), lr, weight_decay=weight_decay)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, data in enumerate(tqdm(train_loader)):
            ...
            query = sketch2xat_student.forward_features(sketch_q)
            with torch.no_grad():
            ...
            loss = loss + xmrd_angle(photo, sketch_q, sketch_k, teacher, sketch2xat_student, modality='sketch')
        ...
        if epoch % 100 == 0:
            print('Updating Sketch-to-XAT student checkpoint...')
            torch.save(sketch2xat_student.state_dict(),
                       p.sketch_student_tgt_dir + '/sketch_student_' + str(epoch) + '.pth')
